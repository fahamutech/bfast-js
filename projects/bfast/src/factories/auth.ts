...
export class DefaultAuthFactory implements AuthAdapter {
  ...
  async logIn<T extends UserModel>(username: string, password: string, appName: string, options?: AuthOptions): Promise<T> {
    ...
    Object.assign(authRule, {
      applicationId: this.config.credential(appName).applicationId
    });
    Object.assign(authRule, {
      auth: {
        signIn: {
    ...
    const response = await this.restApi.post<T>(this.config.databaseURL(appName), authRule, {
      headers: {
        'x-parse-application-id': this.config.credential(appName).applicationId
      }
    });
  ...
  async signUp<T extends UserModel>(username: string, password: string,
                                    attrs: { [key: string]: any } = {}, appName: string, options?: AuthOptions): Promise<T> {
    const authRule = {};
    Object.assign(authRule, {
      applicationId: this.config.credential(appName).applicationId
    });
    ...
    if (data && data.auth && data.auth.signUp) {
      ...
      throw {
        message: data.errors && data.errors.auth && data.errors['auth.signUp']
          ? data.errors['auth.signUp'].message : 'Fails to signUp'
      };
    }
  }
  ...
  updateUser<T extends UserModel>(id: string, attrs: object, options?: AuthOptions): Promise<any> {
    ...
    //         this.config.databaseURL(this.appName, '/users/' + user.objectId),
    //         userModel, {
    //             headers: postHeaders
    //         });
  ...
  requestEmailVerification<T extends UserModel>(email: string, appName: string, options?: AuthOptions): Promise<T> {
    throw new Error('Not supported yet use your implementation');
  }
